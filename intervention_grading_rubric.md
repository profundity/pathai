...
**Check dose adequacy before invoking compensation.** Before attributing a negative pharmacological result to network compensation, check whether the dose tested was actually at or above the compound's known IC50 for its target(s)/isoform(s). A sub-saturating dose explains a null result without needing compensation at all — that's a simpler, cheaper-to-verify explanation, and it should be ruled out first. (Worked case: MK2206 and GDC-0068, tested at 2.5μM and 1μM respectively against AKT1-3, PMID 37931033 — both doses were 38-313x each compound's own IC50 across all three paralogs, ruling out under-dosing as the explanation for the largely null proliferation result seen in most of the 6 cell lines tested.)
